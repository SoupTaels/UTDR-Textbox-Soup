...
#region UI
	...
	Button = function(datastruct_ = { }) constructor {
		...
		static update = function() {
			...
			if ( range_within(mouse_x_gui, bbox.left - leeway, bbox.right + leeway) && range_within(mouse_y_gui, bbox.top - leeway, bbox.bottom + leeway) ) { //If the mouse has enter within the bounding box
	...
	var i = 0, spr_ = spr_border_octagon;
	butt[i] = new Button({ id_: i, text: "Dialogue [spr_gui_icons,0]", x: 320, y: 12, ystart: 12, padd_multi: 14, sprite: spr_, color_butt: c_orange, color: c_orange, on_enter: undefined, on_leave: undefined, on_click: undefined });
	with ( butt[i].data ) {
		on_click = function() { on_click_(); } on_enter = function() { on_enter_(); } on_leave = function() { on_leave_(); }
	} i++;
	
	butt[i] = new Button({ id_: i, text: "Portrait [spr_gui_icons,1]", x: 320, y: 12, ystart: 12, padd_multi: 14, sprite: spr_, color_butt: c_orange, color: c_orange, on_enter: undefined, on_leave: undefined, on_click: function() { audio_play_sound(snd_select, 0, false); } });
	with ( butt[i].data ) {
		on_click = function() { on_click_(); } on_enter = function() { on_enter_(); } on_leave = function() { on_leave_(); }
	} i++;
	
	butt[i] = new Button({ id_: i, text: "Border      [spr_gui_icons,2]", x: 320, y: 12, ystart: 12, padd_multi: 14, sprite: spr_, color_butt: c_orange, color: c_orange, on_enter: undefined, on_leave: undefined, on_click: function() { audio_play_sound(snd_select, 0, false); } });
	with ( butt[i].data ) {
		on_click = function() { on_click_(); } on_enter = function() { on_enter_(); } on_leave = function() { on_leave_(); }
	} i++;
	
	butt[i] = new Button({ id_: i, text: "About       [spr_gui_icons,3]", x: 320, y: 12, ystart: 12, padd_multi: 14, sprite: spr_, color_butt: c_orange, color: c_orange, on_enter: undefined, on_leave: undefined, on_click: function() { audio_play_sound(snd_select, 0, false); } });
	with ( butt[i].data ) {
		on_click = function() { on_click_(); } on_enter = function() { on_enter_(); } on_leave = function() { on_leave_(); }
	} i++;
