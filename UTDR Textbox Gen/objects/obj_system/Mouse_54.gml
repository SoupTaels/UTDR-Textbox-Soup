...
credits_add("Undertale, Deltarune: Toby Fox, Temmie Chang", "https://undertale.com/about/");
credits_add("Made in GameMaker", "https://gamemaker.io/");
credits_add();
credits_add("Huge thanks to JujuAdams especially as this wouldn't\nhave been possible without his tools!");
credits_add();
...
array_push(arr_, new LuiRow().setFlexJustifyContent(flexpanel_justify.center).addContent([
	new LuiImage({ value: spr_soul, color: #ed4577, draw_normal: true, }).setSize(20, 16).addEvent(LUI_EV_MOUSE_LEFT_PRESSED, function(element_) { element_.main_ui.animate(element_, "xscale", 1, 0.15, , 1.3); element_.main_ui.animate(element_, "yscale", 1, 0.15, , 1.3); sfx_play(snd_bump); }),
	new LuiImage({ value: get_icon("tinysoupy"), draw_normal: true,  }).setSize(19, 19).addEvent(LUI_EV_MOUSE_LEFT_PRESSED, function(element_) { element_.main_ui.animate(element_, "xscale", 1, 0.15, , 0.5); element_.main_ui.animate(element_, "yscale", 1, 0.15, , 1.5); sfx_play(snd_squish); }),
	new LuiImage({ value: spr_soul, color: #ed4577, draw_normal: true, }).setSize(20, 16).addEvent(LUI_EV_MOUSE_LEFT_PRESSED, function(element_) { element_.main_ui.animate(element_, "xscale", 1, 0.15, , 1.3); element_.main_ui.animate(element_, "yscale", 1, 0.15, , 1.3); sfx_play(snd_bump); }),
]));
