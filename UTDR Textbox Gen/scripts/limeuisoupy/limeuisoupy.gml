...
function soupy_message(textarr_ = ["Test", "Test 2"], textbutt_ = "OK", width = 620, height = -1, padd_ = 5, snd_ = snd_dimbox, font_ = fnt_determination, func_ = function(){}) {
	...
	array_push(arr_arr, new LuiButton({ text: textbutt_, "height": 35, font: font_, }).setData("container", containter_).setData("func", func_).setPadding(padd_).addEvent(LUI_EV_CLICK, function (element_) { var myfunc = element_.getData("func"); myfunc(); var maincan = element_.getData("container"); maincan.destroy(); obj_system.ui_paused = false; }));
	panel_.addContent([
...
///@param {Asset.GMSound} snd_ Sound
///@param {Asset.GMFont} font_ Text Font
function soupy_popup(elementsarr, func_ = function(){}, textbutt_ = "OK", width = 620, height = -1, padd_ = 5, snd_ = snd_dimbox, font_ = fnt_determination) {
	window_set_cursor(cr_default);
	if ( !UI_MESSAGE ) { exit; }
	sfx_play(snd_);
	var containter_ = new LuiBox({ x: 0, y: 0, }).centerContent().setPositionAbsolute().bringToFront().setFullSize(); //Fullscreen opaque box
